---
title: "Capítulo 2: O Sistema Ving Tsun"
sidebar:
  label: "Cap. 2: O Sistema Ving Tsun"
description: "O Sistema VT em profundidade — os 6 domínios, a sugestão trilógica, variação como princípio organizador, genealogia e denominação."
---

import { Aside } from '@astrojs/starlight/components';

:::note
**Meta:** ~5500 palavras / 22 páginas. **Fontes:** HT (todas as seções), AL (sistema das diferenças), AL2 (lista como ferramenta), VT (diagrama), TQ (estrutura de titulação).
:::

:::note[Estrutura planejada]
- O que é um sistema (definição geral → aplicação ao VT)
- Considerações sobre o Sistema Ving Tsun
- Os 6 domínios como "listagens" (SNT, CK, BJ, MFJ, LDBG, BJD) — visão panorâmica
- A sugestão trilógica de interpretação
- "O único sistema possível é da variável, já que o conflito é constantemente mutável"
- Completude: cada domínio existe em relação aos outros
- A noção de variação como princípio organizador
- Considerações sobre a Genealogia do Sistema Ving Tsun (contexto histórico)
- Considerações sobre a Denominação Moy Yat Ving Tsun
- Vida-Kung Fu, Sam Faat e Família Kung Fu mencionados brevemente aqui, aprofundados nos caps 6 e 7
:::

> *"Kung Fu sem sistema não é Kung Fu. Kung Fu que depende de sistema, não é bom Kung Fu."*
>
> ~ Grão-Mestre Moy Yat

Se Kung Fu não tem tradução exata para o português ou para o inglês, sistema também é um termo vago. Quase qualquer coisa que tenha várias partes pode ser chamada de sistema.

O primeiro a intuir essa questão foi Leibniz. Na sua Monadologia, propunha que as coisas em si têm pouco valor, o que vale é a relação entre elas. Cada mônada reflete o universo inteiro a partir do seu ponto de vista, e é nessa rede de reflexos que a realidade se constitui.

Porém, graças provavelmente a Descartes, o ocidente passou séculos dividindo tudo em partes cada vez menores na tentativa de reduzir as coisas ao conjunto de suas partes. Funciona para sistemas simples como os da física clássica, porém já falha para sistemas biológicos do dia a dia.

Esse trabalho de pensar em sistemas como um todo só seria recuperado no século XIX e aproveitado por Bertalanffy já em meados do século XX. Ou seja, pensar os sistemas ainda se encontra nos primeiros estágios no ocidente.

No outro lado do mundo, a China, devido às suas convulsões internas, teve sempre um terreno mais instável, porém sem rupturas bruscas de suas tradições de pensamento. Apoiar-se nas variáveis era a forma mais eficiente de transitar em um cenário em constante evolução. O pensamento chinês nasce sistêmico por natureza, não se valoriza as partes em detrimento do todo. É curioso como isso perpassa até as estruturas familiares: o nome da família vem antes do indivíduo.

<Aside type="caution" title="TODO">
Expandir a distinção pensamento chinês (relacional) vs ocidental (elementar). Ver cap 4 para não duplicar.
</Aside>

## O que é o Ving Tsun Kuen (詠春拳)?

O Ving Tsun em chinês será tratado como Ving Tsun Kuen Sut (詠春拳術) ou Ving Tsun Kuen To (詠春拳套). Sut (術) remete à arte, ao método, à técnica. To (套) remete à forma, ao conjunto de movimentos.

O uso do chinês aqui é proposital (como sempre). O termo Kuen no jargão marcial faz referência ao que chamaríamos de estilo no ocidente. Ving Tsun é o nome de sua fundadora que o organizou e legou às gerações seguintes. Herança à qual humildemente me inscrevo a partir das portas que meu Si Fu abriu.

Estilo remete à ideia da forma como algo é praticado, realizado. No jargão marcial acaba indo para as famílias de cada linhagem. Porém aqui vamos além. Essa organização, à primeira vista, trata-se de uma lista de dispositivos simbólicos e corporais que se inter-relacionam.

Um dos pontos centrais do pensamento chinês que abordaremos nesse livro é o de não se fixar nos elementos, mas sim nesse espaço de relacionamento entre eles, onde o movimento de fato ocorre. Ou seja, a lista tem sua importância, mas a forma como os itens se relacionam é que deveria estar em foco.

Como estamos falando de relacionamentos entre itens, podemos chamar isso de sistema. A questão é que um sistema que resolva nele mesmo tende ao esquecimento, à baixa variabilidade, à baixa flexibilidade de aplicações. Neste caso o próprio sistema possui mecanismos de propagação e salvaguarda própria (família).

A esse conjunto de relacionamentos, listas, movimentos, entidades, pessoas chamaremos de Sistema Ving Tsun.

## Por que o Sistema Ving Tsun?

Todo sistema marcial tem como objetivo inicial treinar soldados. Esse fato corrobora a ideia de que o sistema foi concebido para a maestria do uso das facas.

Dizer que o Sistema Ving Tsun foi concebido para desenvolvimento de Kung Fu seria prematuro e sem base na própria genealogia familiar.

O que Si Taai Gung deixou registrado em pedra na sua coleção de Kuen Kuits (拳訣) foi que Leung Bok Toa atribuiu a sua esposa Yim Ving Tsun a organização do sistema e transmissão.

Yim Ving Tsun o recebeu de Ng Mui para resolução de uma opressão a que estava sendo submetida. Está dito no registro genealógico que a natureza desse sistema é ser passado para a geração posterior.

Por outro lado, sistemas não precisam ter uma natureza em si. Cabe aos seus operadores atribuírem valor e uso. Na nossa genealogia, ao menos do contato indireto que tive por meio das relações familiares com anteriores, ficou esse legado de que o Ving Tsun é o melhor sistema para desenvolvimento do Kung Fu. Si Taai Gung Moy Yat inclusive criaria a Moy Yat Ving Tsun com o compromisso de passar o sistema na integralidade, da mesma forma que o recebeu.

Aqui não há juízo de valores. Utilizar o sistema para estudo do próprio Kung Fu não é melhor do que utilizá-lo para obter vantagens, seja em combates reais ou mesmo guerras.

A questão é que o desenvolvimento de um bom Kung Fu é mais versátil. Ter um bom Kung Fu nos habilita a fazer qualquer coisa, inclusive lutar, parafraseando Si Taai Gung novamente.

A mágica sistêmica ocorre quando, ao estudar o sistema para desenvolvimento do próprio Kung Fu, acabamos nós mesmos tendo que passá-lo adiante. Ou seja, para desenvolvimento da ferramenta acabamos nós mesmos moldados por ela.

> *"We shape our tools, and thereafter our tools shape us."*
>
> ~ John Culkin, "A Schoolman's Guide to Marshall McLuhan" (Saturday Review, 1967)

:::note[Genealogia da frase]
- Churchill (1943): "We shape our buildings; thereafter they shape us." — discurso sobre a reconstrução da House of Commons.
- McLuhan (1964): a ideia de que extensões tecnológicas alteram percepção e pensamento (Understanding Media), mas nunca escreveu essa frase exata.
- John Culkin (1967): sintetizou McLuhan com a estrutura retórica de Churchill. É a versão com "tools".
- Deleuze/Guattari (1980): ideia análoga mas mais radical em Mil Platôs, Platô 12 — para eles a co-constituição é simultânea, não sequencial: "Não são as ferramentas que definem o homem, é o inverso: é a máquina, o agenciamento maquínico, que determina o que é ferramenta a cada momento." (p. 494-495, ed. Minuit). Não há "primeiro fazemos" e "depois nos faz" — o agenciamento produz sujeito e ferramenta ao mesmo tempo.
:::

---

## *Encontros de Mestrado*

<Aside type="caution" title="II Encontro (13/03/2026)">
1. Etimologia: "sistema" no grego = "aquilo que se mantém junto"; em chinês hai tong = "reunião de fios"
2. Sistema como input-processamento-output vs sistema chinês como lista/receita (ordem altera o produto)
3. Os 6 domínios e as duas trilogias (SNT/CK/BJ + MFJ/LDBG/BJD)
4. Fases estruturada → semi-estruturada → não-estruturada como continuum, não estados discretos
5. Ensinar vs transmitir vs aprender — Si Fu transmite sistema, ensina cultura, To Dai aprende
6. Sistema é artificial (positivo) — nenhum gesto do VT é natural, tudo foi desenhado
7. Natureza não se prescreve, emerge — não dizer "a natureza do SND é X", deixar brotar
8. "O sistema é mudo" — qualquer interpretação é do praticante, não do sistema
9. Sistema como partitura / maestro — partitura fixa, artista na interpretação
10. Começar dentro do sistema, eventualmente descolar — permissão pessoal
11. Integridade: 6 elementos, nem mais nem menos — adicionar ou remover muda a identidade
12. Sistema aberto vs fechado — núcleo fechado (6 domínios), interpretação aberta
13. 5 gerações genealógicas = um período — arte "antiga" só a partir da 6ª geração
14. Sistema pode ser anterior a Yim Ving Tsun — complexidade improvável para uma pessoa
15. A fundação é "atribuída", não declarada — Leung Bok To nomeou
16. 10ª geração como última conexão necessária com a China — universalização
17. Interdependência, não independência — entrar num sistema já o afeta
18. Fita de Moebius — sistema que retorna a si, dentro vira fora
19. Elementos não-lineares e retroativos — Biu Ji muda como você vê o Siu Nim Tao
20. Transmissor emite puro, não controla recepção — alho/bugalho
21. Sistema vs estilo vs método — Kuen/Kuen-Fa = sistema, Kuen-Sung = estilo
22. Influências confucionista (transmissão/ordem), taoista (movimento/yin-yang), budista (não-dano)
23. Sistema de Kung Fu como lista onde ordem importa
24. San Fat vs Vida Kung Fu — interpretação de Si Taai Gung para não-chineses
25. "Momento programado" — Jullien, moment opportun, paradoxo de agendar o espontâneo
26. Marcialidade como produto primário, desenvolvimento humano como subproduto
27. Chi Sau como alma do sistema — método indireto, desenvolve atributos
28. Mito vs rito — narrativas dão permanência a práticas, não são reivindicações históricas
29. Pré-requisitos amplos = sistema mais eficaz — minimizar barreiras de entrada
30. Escrita como revisitação e treino de articulação — pré-requisito de mestria
</Aside>

<Aside type="caution" title="III Encontro (17/04/2026)">
31. Kung Fu (subjetivo) vs Sistema (objetivo) — Si Fu: sistema é aspecto objetivo, quase material (S5)
32. Marcialidade ≠ luta — sistema exige técnicas de combate, mas marcialidade = combate interno (S16/D5)
33. "O sistema é mudo" / "definir é limitar" — praticante interpreta, sistema não prescreve (S11)
34. Sem sistema, Kung Fu para de evoluir — armadilha do autoaprisionamento (S12)
35. Livro "Fundamentos do Kung Fu, vol.1 Marcialidade" — referenciado diretamente por Si Fu
</Aside>

<Aside type="caution" title="IV Encontro (03/07/2026)">
36. "Domínio" e "território" são termos da família Moy Yat; as fases estruturada, semiestruturada e não estruturada vêm da família Moy Yat Sang. Não são universais do sistema, e podemos usar desde que saibamos de onde vêm
37. Domínios interdependentes, não independentes — a posição que se desenvolve no Siu Nim Tau é o que permite a variação do Cham Kiu. Uma prepara a outra
38. "Estacionário" no lugar de "estático" no Siu Nim Tau — troca que o Si Fu fez na formulação do Mestre Márcio
39. Hai Tong é lista onde a ordem é essencial — uma receita de bolo é lista em que a ordem dos ingredientes não importa; o Hai Tong é o oposto, trocar a ordem muda o sistema
40. Ving Tsun como sistema iniciático de desenvolvimento de Kung Fu estruturado a partir de uma família — se o sistema fosse só as listas, bastaria passar a lista. Sem a relação familiar ninguém recebe a chave para lê-la, e por isso o sistema nasce quando o nome passa de um para outro, individualmente
41. "Ving Tsun" como nome da fundadora é leitura problemática — para o Si Fu soa mais como nome de família ou nome iniciático. Deixou em aberto, dizendo que nunca tinha escutado dessa forma e que isso não quer dizer que esteja errado
42. História e nome do sistema pertencem ao capítulo de genealogia — no capítulo do sistema, o foco é o sistema
</Aside>
