# TODO — Ving Tsun: A System Guide

Pontos em aberto para escrita, pesquisa e discussão com Si Fu.
Atualizado: 2026-04-01.

---

## Status geral

| Capítulo | Status | ~Palavras | ~Páginas | Meta (~páginas) |
|----------|--------|-----------|----------|-----------------|
| 00 - Prefácio | 90% | 326 | 1,3 | 3-4 |
| 01 - Kung Fu | 60% | 1100 | 4,4 | 22 |
| 02 - Sistema Ving Tsun | Planejado | 0 | 0 | 32 |
| 03 - A Arte da Lista | Planejado | 0 | 0 | 20 |
| 04 - Pensamento Sistêmico | Planejado | 0 | 0 | 22 |
| 05 - VT como Educador Sistêmico | Planejado | 0 | 0 | 24 |
| 06 - Vida Kung Fu / Sam Faat | Planejado | 0 | 0 | 22 |
| 07 - Família Kung Fu | Planejado | 0 | 0 | 22 |
| 08 - VT Guia Sistêmico | Planejado | 0 | 0 | 24 |
| 09 - Conclusão | Planejado | 0 | 0 | — |

---

## Capítulo 00 — Prefácio

### Pendente
- [ ] **Como ler este livro** — capítulos independentes mas com arco
- [ ] **Convenções** — termos em cantonês, referências às fontes

### Fontes disponíveis
Suficiente para escrever sem pesquisa adicional.

---

## Capítulo 01 — Kung Fu

### Escrito
- Mou Seut vs Kung Fu (o mal-entendido)
- Etimologia dos ideogramas 功夫
- O que NÃO está nos ideogramas
- Inversão: artes marciais contêm Kung Fu, não o contrário
- Peimin Ni: cardápio vs comida, neo-confucianos Song
- Jullien: ação vs transformação, potencial da situação
- Si Fu: jardineiro, Si Gung: não ofertar a derrota
- Presença da morte e velocidade de aprendizado
- Chaves de abertura, conexão total
- Agressividade vs violência vs marcialidade
- Feminilização da guerra (Galvany, Imperador Amarelo, Sun Tzu, Yim Ving Tsun)

### Pendente — este capítulo

- [ ] **Ideogramas de Chi Wu** — academia que salientou a associação de Kung Fu às artes marciais (final séc XIX / início séc XX). Procurar ideogramas e contexto histórico.

- [ ] **Citação Moy Yat sobre artes Mou / atividades femininas** — procurar no livro *The Legends of Kung Fu Master* a citação sobre artistas marciais deverem exercer atividades mais leves (caligrafia, pintura). Verificar termo correto (artes Mou?).

- [ ] **Por que desenvolver Kung Fu?** ⚠️ Discutir com Si Fu
  - O interesse pelo adversário (feminização, conexão) leva ao desenvolvimento humano
  - Si Gung: "inteligência marcial" = inteligência estratégica cultivada na experiência marcial
  - Si Gung: "a nossa humanidade é o primeiro passo para uma perspectiva trans-individual"
  - Combate como experiência mobilizadora: estender para a conduta o que se tomou consciência a partir do outro
  - Conectar para entender o outro, no processo entender a si mesmo
  - Moy Yat: "Kung Fu não é para a luta. O melhor lutador não é o que tem melhor habilidade, mas sim aquele que tem melhor comportamento e boas maneiras"
  - **Fontes:** playlist `GgoD2DXI3ro` e `Ea0UoCcdeMc` (inteligência marcial). Precisa de mais material do Si Fu.

- [ ] **A arte de viver bem a própria vida** ⚠️ Discutir com Si Fu
  - Neo-confucianos Song: Kung Fu como arte de viver em geral
  - Confucionistas, taoistas e budistas: seus ensinamentos como diferentes escolas de Kung Fu
  - Peimin Ni / Pierre Hadot: "se a filosofia é um caminho de vida, a abordagem Kung Fu sugere que tomemos a filosofia como a busca da arte de viver bem"
  - Moy Yat: "existe para melhorar sua vida"
  - Orientação predominante do pensamento clássico chinês: como devemos viver nossas vidas
  - **Fontes:** playlist `ivcoMBfHm3A` (O que é Kung Fu). Suficiente para um primeiro rascunho.

- [ ] **Ponte para o capítulo 2** — como o Ving Tsun organiza esse processo
  - Incluir: sistema como membrana de preservação (JKD como contra-exemplo)

### Pontos da reunião Programa de Mestrado (11/03/2026) ⚠️ Discutir com Si Fu

Fonte: `fontes/Programa de Mestrado - 2026_03_11 [...] Sobre Kung Fu.md`

- [ ] **Pessoa (人) vs Humano (仁)** — desenvolvimento pessoal é individual, desenvolvimento humano ocorre através da conexão com o outro. Ideograma 仁 = duas pessoas.
- [ ] **Alteroconhecimento** — conhecimento adquirido a partir do outro. Consciência = "com" (conjunto) + "ciência" (saber).
- [ ] **Kung Fu como prudência** — executar da melhor forma com menor gasto de energia. Parcimônia, previsão, antecipação. Analogia do carro que percorre mais com menos combustível.
- [ ] **Dilema dedicação vs eficiência** — quem tem mais Kung Fu: quem se dedicou mais ou quem fez melhor com menos? Sem resposta única. Demonstra a complexidade do conceito.
- [ ] **Atenção e tensão** — attendere (esticar-se para). Tensão e relaxamento não são bons/ruins, dependem da medida. Corda do instrumento: frouxa não soa, esticada arrebenta.
- [ ] **Desenvolvimento como retirada de camadas** — des-envolvimento = retirar envolvimentos que atrapalham a essência. Inicia com consciência, frequentemente através do contato com o outro.
- [ ] **Excelência como movimento, não estado** — diferente de perfeição (abstração). Língua chinesa foca no movimento (ir para/ir de) mais que no estado (estar em).
- [ ] **Desfrute e hedonismo** — Kung Fu bem vivido como fonte de prazer. Hedonismo = extrair prazer do que se tem, não busca incessante. Wu Wei / fluir.
- [ ] **Universalidade do Kung Fu** — como futebol (inglês → universal). Preservar o simbólico, evitar "chinesismos". Aprender novos idiomas ativa áreas diferentes do cérebro.
- [ ] **Kung Fu como conceito + processo + resultante** — trabalham concomitantemente. Conceito vem de tradições, processo via formalidades sistêmicas, resultante gera novas inteligências.
- [ ] **Maturidade como alvo móvel** — na filosofia, maturidade é ponto preliminar da decadência. Ser humano em "atualização" constante, nunca "pronto". Acreditar-se pronto transforma humano em objeto.

### Pendente — outros capítulos (nasceu aqui, mora lá)

- [ ] **Yin/Yang como estrutura do Kung Fu** → Cap 04 ou 05
  - Bruce Lee: complementaridade (não dualismo), firmeza na suavidade, bambu vs árvore rígida
  - "Cultivation of martial art in the long run will return to simplicity"
  - Fonte: `THE_BASIC_THEORY_OF_YIN_AND_YANG_IN_THE_ART_OF_GUNG_FU.pdf`

- [ ] **Shuhari (守破離)** → Cap 05 ou 06
  - Shu: manter fundamentos | Ha: quebrar a forma | Ri: liberdade
  - "É formless trazer criatividade sem entender a forma básica"
  - Fonte: `shuhari - project_diary_138 (1).pdf`

- [ ] **Lógica de Modelo vs Lógica de Desenvolvimento** → Cap 04
  - Modelo (ocidental): querer → plano → meta → ação → esforço → herói
  - Desenvolvimento (chinesa): avaliar cenário → antecipar → Wu Wei → adaptação
  - "Solução de hoje é o problema de amanhã"
  - "A não-ação não é contraposição à ação, é antecipação da ação"
  - Fontes: playlist `gc2yXK6ae2c` (Lógica de Modelo) e `DJfIXrZe_3s` (Lógica de Desenvolvimento)

---

## Capítulo 02 — O Sistema Ving Tsun

### Pendente
- [ ] Definição de sistema → aplicação ao VT
- [ ] Os 6 domínios (SNT, CK, BJ, MFJ, LDBG, BJD) como listagens panorâmicas
- [ ] Modelo de interpretação trilógica
- [ ] Variação como princípio organizador
- [ ] Completude: cada domínio em relação aos outros
- [ ] Genealogia do sistema (contexto histórico)
- [ ] Denominação (Moy Yat Ving Tsun)

### Fontes disponíveis
`Ving Tsun Kuen Sut.pdf`, `HAI TONG por MOY YAT.pdf`, `VTExp2022 - SND - Listagem.pdf`, `SND - Linha Central.pdf`, `BJ - Apresentação1.pdf`. Suficiente para iniciar.

---

## Capítulo 03 — A Arte da Lista

### Pendente
- [ ] Tradição chinesa de codificar por listas (dinastia Tang)
- [ ] Lista ≠ inventário: instrumento de variação
- [ ] "Nenhum perigo maior do que se imobilizar num caso dado"
- [ ] Listas do VT: kuen to como listagens de movimento
- [ ] Como ler uma lista: Sam Faat e a ordem interna invisível
- [ ] Paralelo: lista como sistema (elementos, relações, emergência)

### Fontes disponíveis
`A Arte da Lista no Sistema Ving Tsun.pdf`, `A Arte da Lista - A Trilogia Fundamental.pdf`, `O Sistema de Kung Fu e a Arte da Lista.pdf`. Fontes ricas, prontas para escrita.

---

## Capítulo 04 — Pensamento Sistêmico (Ocidente e Oriente)

### Pendente
- [ ] Reducionismo vs Holismo
- [ ] Ocidente: Bertalanffy → Checkland → soft/hard systems
- [ ] Oriente: holismo enraizado (Confucionismo, Taoismo, Budismo)
- [ ] Convergência: unidade dialética
- [ ] Yin-Yang como pensamento sistêmico nativo
- [ ] Meta-síntese e WSR: metodologias chinesas
- [ ] Wuli-Shili-Renli
- [ ] Papel da intuição (diferença China/Ocidente)

### Fontes disponíveis
`Systems Thinking - A Comparison between Chinese and Western.pdf` (artigo acadêmico, fonte principal). Lógica de Modelo/Desenvolvimento (playlist). Pode precisar de mais fontes ocidentais (Bertalanffy, Checkland, Senge).

---

## Capítulo 05 — VT como Educador Sistêmico

### Pendente
- [ ] Holismo na prática: san-sau-ma como sistema unitário
- [ ] Emergência: ging 勁 não presente nas partes isoladas
- [ ] Feedback loops: Chi Sau como sistema de feedback contínuo
- [ ] Hierarquia e subsistemas: SNT → CK → BJ como camadas
- [ ] Biu Ji: retorno ao equilíbrio como propriedade emergente
- [ ] Mudanças quantitativo-qualitativas na prática
- [ ] Shu-Ha-Ri aplicado ao VT

### Fontes disponíveis
Material técnico VTExp + artigo Systems Thinking. Pode precisar de mais fontes sobre emergência e feedback em sistemas.

---

## Capítulo 06 — Vida Kung Fu / Sam Faat

### Pendente
- [ ] O que é Vida Kung Fu: prática como modo de vida
- [ ] Sam Faat 心法 — transmissão intuitiva, mente a mente
- [ ] Ordem interna invisível das listas
- [ ] Sam Faat na vida: ler contextos além da conceituação racional
- [ ] Diferença entre explicar e fazer (conhecimento incorporado)
- [ ] Kung Fu amplo: cozinhar, liderar, criar

### Fontes disponíveis
Arte da Lista (Sam Faat), Hai Tong, playlist (Vida Kung Fu). Precisa de material do Si Fu sobre Sam Faat na prática.

---

## Capítulo 07 — Família Kung Fu e Genealogia

### Pendente
- [ ] Si Fu, To Dai, Si Hing Dai — estrutura familiar
- [ ] Genealogia como "árvore de transmissão"
- [ ] Si Fu como nó central: responsabilidade, não autoridade
- [ ] WSR no VT: Wuli (técnica), Shili (método), Renli (relações)
- [ ] Harmonia como objetivo sistêmico
- [ ] Processo de transmissão entre gerações
- [ ] Histórias Moy Yat / Ip Man

### Fontes disponíveis
Hai Tong, Systems Thinking (WSR), Entrevista Si Suk Ursula. Pode precisar de mais material genealógico e histórias pessoais.

---

## Capítulo 08 — VT como Guia para o Pensamento Sistêmico

### Pendente
- [ ] Tese central reunificada
- [ ] Aplicações além do combate
- [ ] Casos: praticantes aplicando VT nas profissões
- [ ] Convergência ocidente/oriente
- [ ] Pensamento sistêmico não se aprende lendo

### Fontes disponíveis
Todas convergem aqui. Precisa de casos/depoimentos de praticantes.

---

## Capítulo 09 — Conclusão

### Pendente
- [ ] Recapitulação do arco (sem repetir)
- [ ] O que o livro não disse
- [ ] A jornada como resultado
- [ ] Reflexão pessoal do autor
- [ ] Agradecimentos

---

## Pesquisa pendente (global)

- [ ] OCR do `the hidden power of kung fu.pdf` (scan sem texto)
- [ ] Livro **Marcialidade** (Julio Camacho, Ursula Lima, Ricardo Queiroz) — conseguir acesso ao conteúdo
- [ ] Mais material do Si Fu sobre desenvolvimento humano via combate
- [ ] Fontes ocidentais sobre pensamento sistêmico (Bertalanffy, Checkland, Senge) para cap 04
- [ ] Casos/depoimentos de praticantes para cap 08
